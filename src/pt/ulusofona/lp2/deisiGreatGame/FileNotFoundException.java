...
public class FileNotFoundException extends Abyss {
    public FileNotFoundException(int id, String titulo, int numeroCasa) {
        super(id, titulo, numeroCasa);
    }

    @Override
    public String reactToSquare(Programmer jogador, GameManager jogo) {
        jogo.addCountAbismosPassados(3);
        ...
        if (jogador.getFerramentas() != null) {
            for (int i = 0; i < jogador.getFerramentas().size(); i++) {
                if (jogador.getFerramentas().get(i).getId() == 5 || jogador.getFerramentas().get(i).getId() == 3) {
                    ...
                    jogo.trocaTurno();
                    return "Escapaste do abismo: File Not Found Exception - A ferramenta: " + nomeFerramenta + " foi retirada do seu inventário!";
                }
            }
        }
        if ((jogador.getPosicao()) > 3) {
            jogo.getCurrentBoard().get(jogador.getPosicao()).remove(jogador);
            jogador.retirarPosicao(3);
        ...
        jogo.trocaTurno();
        return "Caíste no abismo: File Not Found Exception - Volte 3 casas";
    }
}
